...
<!-- ── Packages used in this chapter ──────────────────────────────────
     Rendered (knitr / build-time):  none (this chapter has no setup chunk)
     WebR (in-browser cells):         ggplot2, dplyr
       WebR packages are declared in the YAML `webr: packages` block above
       so they load when the deck opens (see help.html).
────────────────────────────────────────────────────────────────────── -->
...
- [The 2nd and 4th elements: 75 and 88]{.correct data-explanation="R indexes from 1, so position 2 is 75 and position 4 is 88. c(2, 4) selects both of those positions."}
...
- [Data frame]{.correct data-explanation="A data frame is a table where each row is one observation and each column is one variable, and columns may hold different types - the standard structure for applied statistics."}
- Numeric vector
- Matrix
- Factor
...
- [Call `library(ggplot2)` - packages must be loaded each session]{.correct data-explanation="install.packages() downloads a package once. library() loads it into each new session, and must be run before the package's functions are available."}
- Run `install.packages("ggplot2")` again
- Nothing - installed packages load automatically every session
- Reinstall R
...
:::: {.three-col}
::: {.card .fragment}
<span class="label">R & RStudio</span>
**R** is an open-source language for statistics, data analysis, and graphics.<br>**RStudio** is the interface for writing scripts, running code, and viewing plots.<br>**Scripts** make analyses easy to save, reproduce, and rerun.
:::
::: {.card .fragment}
<span class="label">Data Structures</span>
**Variables** store values or objects; **vectors** store multiple values of one type.<br>**Lists**, **matrices**, **arrays**, **factors**, and **data frames** organize richer data.<br>**Indexing** with `[ ]` selects elements, rows, and columns.
:::
::: {.card .fragment}
<span class="label">Functions & Packages</span>
**Functions** like `mean()`, `sort()`, and `summary()` reuse common operations.<br>**Packages** extend R: `install.packages()` once, `library()` each session.<br>**Base graphics** and **`ggplot2`** both create statistical plots.
:::
::::
