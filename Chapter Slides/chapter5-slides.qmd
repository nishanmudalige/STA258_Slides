...
<span class="label">USA-Canada hockey example</span>
...
## Check in R: Pooled Two-Sample t Intervals
...
n1 <- 12; x1 <- 52.4; s1 <- 6.1
n2 <- 10; x2 <- 47.8; s2 <- 5.4
...
ci <- (x1 - x2) + c(-1, 1) * tstar * se

round(c(diff = x1-x2, sp = sp, SE = se,
        df = df, tstar = tstar,
        lower = ci[1], upper = ci[2]), 3)
```
:::
...
## Example: UFC Highlight Watch-Time Comparison

:::: {.example-grid style="display: grid; grid-template-columns: 0.87fr 1.13fr; grid-template-rows: auto auto; gap: 10px 18px; align-items: stretch;"}

::: {.taskbox style="grid-column: 1; grid-row: 1; font-size: 0.63em; line-height: 1.16; padding: 11px 15px; margin-bottom: 0;"}
A UFC media team compares how long viewers spend watching two fighter
highlight playlists.

The Khabib playlist has shorter, more focused grappling highlights, while
the Jon Jones playlist has longer mixed striking, wrestling, and title-fight
clips.

<br>

**Group 1: Khabib highlight viewers**  
$n_1=24,\quad \bar{x}_1=9.8,\quad s_1=3.1$ minutes

**Group 2: Jon Jones highlight viewers**  
...
$\mu_1=$ mean watch time for Khabib highlight viewers

$\mu_2=$ mean watch time for Jon Jones highlight viewers

The order is **Khabib minus Jon Jones**.
:::

::: {.examplebox style="grid-column: 2; grid-row: 2; font-size: 0.66em; line-height: 1.16; padding: 12px 15px; margin-bottom: 0; display: flex; align-items: center;"}
We are 95% confident that the Khabib playlist's population mean watch time
is between **5.82 minutes lower and 0.22 minutes higher** than the Jon Jones
playlist's population mean watch time. Since the interval crosses 0, the
data do not show a clear difference in mean watch time.
:::
...
## Check in R: Welch Two-Sample t Intervals

:::: {.two-col-wide style="grid-template-columns: 1fr 1fr; gap: 18px; align-items: start;"}

::: {.col}
::: {.notebox style="font-size: 0.58em; line-height: 1.10; padding: 8px 11px; margin-bottom: 6px;"}
<span class="label">Food-delivery wait-time example</span>
:::
...
::: {.col}
::: {.notebox style="font-size: 0.58em; line-height: 1.10; padding: 8px 11px; margin-bottom: 6px;"}
<span class="label">UFC highlight watch-time example</span>
:::
...
## Example: Free-Trial Reminder Comparison

:::: {.example-grid style="display: grid; grid-template-columns: 0.87fr 1.13fr; grid-template-rows: auto auto; gap: 10px 18px; align-items: stretch;"}

::: {.taskbox style="grid-column: 1; grid-row: 1; font-size: 0.63em; line-height: 1.16; padding: 11px 15px; margin-bottom: 0;"}
A streaming app wants to compare two ways of reminding users before a
free trial ends.
...
values mean the direct reminder has the higher population continuation
rate.
:::

::: {.examplebox style="grid-column: 2; grid-row: 2; font-size: 0.66em; line-height: 1.16; padding: 12px 15px; margin-bottom: 0; display: flex; align-items: center;"}
We are 95% confident that the direct reminder's population continuation
rate is between **0.69 and 14.17 percentage points higher** than the
general notification's population continuation rate.
:::
...
::: {.col}
::: {.notebox style="font-size: 0.58em; line-height: 1.10; padding: 8px 11px; margin-bottom: 6px;"}
<span class="label">GTA VI trailer alert example</span>
:::
...
## Example: PS5 Restock Checkout-Time Variability

:::: {.example-grid style="display: grid; grid-template-columns: 0.87fr 1.13fr; grid-template-rows: auto auto; gap: 10px 18px; align-items: stretch;"}

::: {.taskbox style="grid-column: 1; grid-row: 1; font-size: 0.63em; line-height: 1.16; padding: 11px 15px; margin-bottom: 0;"}
A retailer wants to compare how inconsistent checkout times are during a
PS5 console restock versus a normal PlayStation accessory purchase.

<br>

**Group 1: Regular accessory checkout**  
$n_1=18,\quad s_1=6$ minutes

**Group 2: PS5 restock checkout**  
...
$$
df_{\text{num}}=n_2-1=15,
\qquad
df_{\text{den}}=n_1-1=17
$$
...
The order is **PS5 restock variability divided by regular accessory
checkout variability**.

Values above 1 mean the PS5 restock checkout times are more variable.
:::

::: {.examplebox style="grid-column: 2; grid-row: 2; font-size: 0.66em; line-height: 1.16; padding: 12px 15px; margin-bottom: 0; display: flex; align-items: center;"}
We are 95% confident that the PS5 restock checkout-time variance is
between **2.00 and 15.31 times** the regular accessory checkout-time
variance. This suggests the PS5 restock process is much less consistent.
:::
...
::: {.col}
::: {.notebox style="font-size: 0.58em; line-height: 1.10; padding: 8px 11px; margin-bottom: 6px;"}
<span class="label">PS5 restock checkout-time example</span>
:::
...
ci <- mean_d + c(-1, 1) * tstar * SE

out <- round(data.frame(
  mean_d = mean_d, sd_d = sd_d, SE = SE,
  df = df, tstar = tstar,
  lower = ci[1], upper = ci[2]
), 3)
...
## Example: MrBeast 500M Giveaway Entry-Time Estimate

:::: {.example-grid style="display: grid; grid-template-columns: 0.87fr 1.13fr; grid-template-rows: auto auto; gap: 10px 18px; align-items: stretch;"}

::: {.taskbox style="grid-column: 1; grid-row: 1; font-size: 0.63em; line-height: 1.16; padding: 11px 15px; margin-bottom: 0;"}
A media analytics team is planning a large MrBeast 500M giveaway page.

Before launch, they want to estimate the **mean time** it takes a subscriber
to complete the giveaway entry form.

From previous giveaway launches, the population standard deviation is
treated as known:
...
The team wants the estimate to be within:
...
mean giveaway entry time.

The margin of error should be no more than **5 seconds**.
:::

::: {.examplebox style="grid-column: 2; grid-row: 2; font-size: 0.66em; line-height: 1.16; padding: 12px 15px; margin-bottom: 0; display: flex; align-items: center;"}
The team should sample at least **89 subscribers** to estimate the mean
giveaway entry time within **5 seconds** at 95% confidence.
:::
...
::: {.taskbox style="grid-column: 1; grid-row: 1; font-size: 0.63em; line-height: 1.16; padding: 11px 15px; margin-bottom: 0;"}
A ride-share app wants to estimate its average pickup-time prediction error.

From past trip data, the population standard deviation of prediction error
is treated as known:
...
::: {.examplebox style="grid-column: 2; grid-row: 2; font-size: 0.66em; line-height: 1.16; padding: 12px 15px; margin-bottom: 0; display: flex; align-items: center;"}
The app should sample at least **840 trips** to estimate the mean pickup-time
prediction error within **2 seconds** at 99% confidence.
:::
...
::: {.col}
::: {.notebox style="font-size: 0.58em; line-height: 1.10; padding: 8px 11px; margin-bottom: 6px;"}
<span class="label">MrBeast entry-time example</span>
:::
...
## Example: Jordan vs LeBron GOAT Rating
...
A sports media page wants to estimate the mean rating fans give to a
Jordan vs LeBron GOAT debate video.

Each fan rates the video on a **0 to 100 scale**.

Since $\sigma$ is unknown, the page uses a pilot study:
...
We are planning a confidence interval for the population mean fan rating.

Because $\sigma$ is unknown, the pilot-study standard deviation $s=14.5$
is used as the preliminary estimate.
:::
...
The sports media page should sample at least **53 fans** to estimate the
mean GOAT-debate video rating within **4 points** at 95% confidence.
:::
...
::: {.col}
::: {.notebox style="font-size: 0.58em; line-height: 1.10; padding: 8px 11px; margin-bottom: 6px;"}
<span class="label">Jordan vs LeBron GOAT rating example</span>
:::
...
out <- data.frame(
  start = n0, iter1 = n1, iter2 = n2, final = n3
)
...
n0 <- ceiling((qnorm(0.95) * s / E)^2)
n1 <- ceiling((qt(0.95, n0 - 1) * s / E)^2)
n2 <- ceiling((qt(0.95, n1 - 1) * s / E)^2)
n3 <- ceiling((qt(0.95, n2 - 1) * s / E)^2)
...
::: {.col}
::: {.notebox style="font-size: 0.58em; line-height: 1.10; padding: 8px 11px; margin-bottom: 6px;"}
<span class="label">Sabrina Carpenter merch survey example</span>
:::
...
::: {.thinkbox .fragment}
**One slide takeaway.** Confidence intervals are all the same basic idea:
estimate the quantity, account for uncertainty, and report a reasonable range of values.
:::
