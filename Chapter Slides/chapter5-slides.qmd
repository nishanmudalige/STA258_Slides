...
$$
\left(
\frac{(n-1)s^2}{\text{upper critical value}},\
\frac{(n-1)s^2}{\text{lower critical value}}
\right)
$$

Here, $s^2$ is the point estimate of $\sigma^2$, while $(n-1)s^2$ is the scaled sample variance used in the interval formula.
:::
...
::: {.examplebox .fragment}
Using $n=10$, $\bar{x}=17.7$, $s=9.0805$, $df=9$, and $t^*=2.262$:

$$
17.7\pm2.262\left(\frac{9.0805}{\sqrt{10}}\right)
=
17.7\pm6.50
=
(11.20,24.20).
...
We are 95% confident that the population mean number of books read is between approximately **11.20 and 24.20 books**.
:::
...
.mini-card .label, .stat-card .label, .warnbox .label, .thinkbox .label {
  display: block;
  color: var(--sta-teal-dark);
  font-weight: 700;
  margin-bottom: 5px;
...
/* ── Tables ───────────────────────────────────────────── */
.reveal table { width: 100%; border-collapse: collapse; border: 1px solid var(--sta-border); font-size: 0.66em; margin-bottom: 16px; }
.reveal th, .reveal td {
  border: 1px solid var(--sta-border);
  padding: 7px 10px;
  vertical-align: top;
...
.reveal th { background: var(--sta-teal-light); font-weight: bold; }
.reveal table td { text-align: center !important; }
.reveal table th { text-align: left !important; }
...
/* ── Balanced worked-example slides ───────────────────── */
...
/* Keep calculations comfortably readable */
...
```{=html}
<script>
function setupWorkedExampleFragments() {
  document.querySelectorAll(".example-grid").forEach(function (grid) {
    const targetCard = grid.querySelector(".notebox");
    const calculationCard = grid.querySelector(".stat-card");
    const interpretationCard = grid.querySelector(".examplebox");

    const revealOrder = [
      { card: targetCard, index: 1 },
      { card: calculationCard, index: 2 },
      { card: interpretationCard, index: 3 }
    ];

    revealOrder.forEach(function (item) {
      if (!item.card) return;

      item.card.classList.add("fragment");
      item.card.setAttribute("data-fragment-index", item.index);
    });
  });
}
...
document.addEventListener("DOMContentLoaded", function () {
  /*
   * Worked-example reveal order:
   * 1. Target parameter or subtraction order
   * 2. Calculation
   * 3. Interpretation
   *
   * The task card remains visible when the slide first opens.
   */
  setupWorkedExampleFragments();
...
/*
 * Bind to reveal.js once it exists. reveal can fire "ready" before this code
 * runs (and window.Reveal may be undefined at DOMContentLoaded), so retry until
 * it is available, then apply the fragment classes and call Reveal.sync() so
 * reveal registers them and reveals the cards one at a time.
 */
(function bindWorkedExampleFragments() {
  setupWorkedExampleFragments();
  ...
  if (window.Reveal && typeof window.Reveal.on === "function") {
    ...
    window.setTimeout(bindWorkedExampleFragments, 150);
  }
})();
</script>
```
...
:::: {.two-col style="margin-top: 14px; align-items: center;"}
::: {.col}
**Mean and proportion differences:** $\text{Group 1}-\text{Group 2}$
:::

::: {.col}
**Variance ratios:** $\dfrac{\text{Group 1}}{\text{Group 2}}$
:::
::::
:::

:::: {.three-col style="gap: 14px; align-items: stretch;"}
::: {.stat-card .fragment style="font-size: 0.72em; line-height: 1.2; padding: 15px 17px; margin-bottom: 0; min-height: 190px;"}
...
Independent random samples of earbuds are selected from the two brands.
Every earbud set is tested under the same conditions: same volume, same
audio, and same room temperature.

Assume battery life is approximately normal within each population and
that the two population variances are reasonable to treat as equal.
...
## Example: Fitness App Calorie Comparison

:::: {.example-grid style="display: grid; grid-template-columns: 0.84fr 1.16fr; grid-template-rows: auto auto; gap: 8px 15px; align-items: stretch;"}

::: {.taskbox style="grid-column: 1; grid-row: 1; margin-bottom: 0;"}
A fitness reviewer randomly assigns 29 participants to one of two
30-minute workouts.

Assume calories burned are approximately normal within each group and
that the population variances can be treated as equal.

<br>

**High-intensity app**  
$n_1=15$, $\bar{x}_1=84.2$, $s_1=7.5$

**Guided routine**  
$n_2=14$, $\bar{x}_2=78.6$, $s_2=6.8$
...
\mu_1-\mu_2
=
\mu_{\text{app}}-\mu_{\text{guided}}
$$

Positive values favour the high-intensity app.
:::

::: {.examplebox style="grid-column: 2; grid-row: 2; margin-bottom: 0;"}
The high-intensity app burns between **1.06 and 10.14 more calories on
average** than the guided routine, with 90% confidence.
:::
...
.reveal section.pooled-r-check .cm-gutters,
.reveal section.pooled-r-check .cm-gutterElement,
...
::: {.notebox .fragment style="font-size: 0.66em; line-height: 1.12; padding: 11px 15px; margin-bottom: 7px;"}
...
::: {.defbox .fragment style="font-size: 0.64em; line-height: 1.10; padding: 11px 15px; margin-bottom: 7px;"}
...
::: {.thinkbox .fragment style="font-size: 0.50em; line-height: 1.08; padding: 7px 12px; margin-bottom: 0;"}
<span class="label">How the examples use these methods</span>

The upcoming **Sports Highlight** example uses the conservative rule
$df=\min(n_1-1,n_2-1)$ for a manageable hand calculation. The **Food
Delivery** example uses the more accurate Welch–Satterthwaite
approximation to illustrate the calculation used by statistical software.

For exams, use the conservative rule unless told otherwise.
:::

------------------------------------------------------------------------

## Example: Sports Highlight Watch-Time Comparison {.grid-compact}

:::: {.example-grid style="display: grid; grid-template-columns: 0.87fr 1.13fr; grid-template-rows: auto auto; gap: 10px 18px; align-items: stretch;"}

::: {.taskbox style="grid-column: 1; grid-row: 1; font-size: 0.63em; line-height: 1.16; padding: 11px 15px; margin-bottom: 0;"}
A sports media team uses two independent random samples of viewers to
compare watch times for two types of highlight playlists.

One playlist has shorter, fast-paced clips, while the other has longer,
more detailed highlight clips.

Assume watch times are approximately normal within each population.
...
## Example: Food Delivery vs Meal Prep Spending {.grid-compact}
...
.reveal section.welch-r-check .cm-gutters,
.reveal section.welch-r-check .cm-gutterElement,
...
::: {.thinkbox style="font-size: 0.49em; line-height: 1.10; padding: 7px 12px; margin-bottom: 7px;"}
<span class="label">Why the R output may differ</span>

**Sports Highlight:** The hand calculation uses the conservative rule
$df=\min(n_1-1,\;n_2-1)$, while R uses the Welch–Satterthwaite
approximation. Its degrees of freedom and interval endpoints will
therefore differ slightly.

**Food Delivery:** The hand calculation already uses the
Welch–Satterthwaite approximation, so its R output should closely match,
apart from harmless rounding differences.
:::
...
## Example: App Notification Click-Through Rates

:::: {.example-grid style="display: grid; grid-template-columns: 0.87fr 1.13fr; grid-template-rows: auto auto; gap: 8px 15px; align-items: stretch;"}

::: {.taskbox style="grid-column: 1; grid-row: 1; margin-bottom: 0;"}
A mobile app randomly selects 950 eligible users and randomly assigns
them to one of two notification methods.

<br>

**Push notification**  
$210$ of $500$ users opened the feature within 24 hours.

**Homepage banner**  
$170$ of $450$ users opened the feature within 24 hours.
...
p_1-p_2
=
p_{\text{push}}-p_{\text{banner}}
$$

Positive values favour the push notification.
:::

::: {.examplebox style="grid-column: 2; grid-row: 2; margin-bottom: 0;"}
The push-notification click-through rate could be between **1.00
percentage point lower and 9.45 percentage points higher** than the
homepage-banner rate.

Because the interval includes **0**, neither method has a clearly higher
click-through rate at the 90% confidence level.
:::
...
.reveal section.prop-r-check .cm-gutters,
.reveal section.prop-r-check .cm-gutterElement,
...
## Example: Sale Checkout-Time Variability {.grid-compact}
...
$$
F_{0.975,15,17}\approx2.723,
\qquad
F_{0.025,15,17}\approx0.356
$$

Using the unrounded critical values,

$$
\left(
\frac{5.4444}{F_{0.975,15,17}},
\frac{5.4444}{F_{0.025,15,17}}
\right)
\approx
(2.00,\;15.31)
$$
...
Assume the two datasets are independent random samples and that charging
times are approximately normal within both populations.
...
.reveal section.ratio-r-check .cm-gutters,
.reveal section.ratio-r-check .cm-gutterElement,
...
:::: {.example-grid style="display: grid; grid-template-columns: 0.88fr 1.12fr; grid-template-rows: auto auto; gap: 8px 15px; align-items: stretch;"}

::: {.taskbox style="grid-column: 1; grid-row: 1; margin-bottom: 0;"}
A random sample of 12 people is observed during two comparable winter
periods:

- **without** regular Vitamin C
- **with** regular Vitamin C

Assume the paired differences in sick days are approximately normally
distributed.

For each person, define

$$
d=\text{sick days without Vitamin C}
-\text{sick days with Vitamin C}
$$
...
$$
d=\text{without Vitamin C}-\text{with Vitamin C}
$$

Positive values mean fewer sick days during the Vitamin C period.
:::

::: {.examplebox style="grid-column: 2; grid-row: 2; margin-bottom: 0;"}
<span class="label">Interpretation</span>

We are 95% confident that the Vitamin C period had between **0.98 and
2.52 fewer sick days on average**.

Because Vitamin C use was not randomly assigned, this shows an
...
A random sample of 12 people tracks their sleep before and after using a
30-minute phone-free routine before bed. Assume the paired differences
...
.reveal section.paired-r-check .cm-gutters,
.reveal section.paired-r-check .cm-gutterElement,
...
## Example: Online Entry-Form Completion Time

:::: {.example-grid style="display: grid; grid-template-columns: 0.87fr 1.13fr; grid-template-rows: auto auto; gap: 10px 18px; align-items: stretch;"}

::: {.taskbox style="grid-column: 1; grid-row: 1; font-size: 0.63em; line-height: 1.16; padding: 11px 15px; margin-bottom: 0;"}
A university is testing a new online entry form and wants to estimate the
population mean time required to complete it.

From extensive historical system data, the population standard deviation
of completion times is treated as known:

$$
\sigma=24\text{ seconds}
$$

The university wants a 95% confidence interval with margin of error no
more than:
...
population mean online entry-form completion time.

The margin of error should be no more than **5 seconds**.
:::

::: {.examplebox style="grid-column: 2; grid-row: 2; font-size: 0.66em; line-height: 1.16; padding: 12px 15px; margin-bottom: 0; display: flex; align-items: center;"}
The university should observe at least **89 form completions** to estimate
the population mean completion time within **5 seconds** at 95%
confidence.
:::
...
.reveal section.mean-size-r-check .cm-gutters,
.reveal section.mean-size-r-check .cm-gutterElement,
...
## Example: Tutorial Video Rating Estimate {.grid-compact}

```{=html}
<style>
.reveal section.grid-compact .example-grid .taskbox,
.reveal section.grid-compact .example-grid .notebox,
.reveal section.grid-compact .example-grid .examplebox { font-size: 0.51em !important; line-height: 1.12 !important; }
.reveal section.grid-compact .example-grid .stat-card { font-size: 0.61em !important; }
.reveal section.grid-compact .example-grid .stat-card mjx-container[display="true"] { margin: 0.42em 0 !important; }
</style>
```

:::::: {.example-grid style="display: grid; grid-template-columns: 0.87fr 1.13fr; gap: 8px 15px;"}

::::: {style="display: flex; flex-direction: column; gap: 8px;"}

:::: {.taskbox style="margin-bottom: 0;"}
An online learning platform wants to estimate the mean rating students
give to a new tutorial video.
...
:::: {.notebox style="margin-bottom: 0;"}
<span class="label">Planning goal</span>

We are planning a confidence interval for the population mean student
rating.

Because $\sigma$ is unknown, the pilot-study standard deviation
$s=14.5$ is used as the preliminary estimate.
::::
...
:::: {.stat-card style="margin-bottom: 0;"}
<span class="label">Calculation</span>
...
53 & 52 & 2.007 & 52.93 \Rightarrow 53
\end{array}
$$
...
:::: {.examplebox style="margin-bottom: 0;"}
The online learning platform should sample at least **53 students** to
estimate the mean tutorial-video rating within **4 points** at 95%
confidence.
::::
...
.reveal section.unknown-size-r-check .cm-gutters,
.reveal section.unknown-size-r-check .cm-gutterElement,
...
.reveal section.prop-size-r-check .cm-gutters,
.reveal section.prop-size-r-check .cm-gutterElement,
...
We shift from estimating a parameter to **testing a claim** about it: is a specific value compatible with the sample data? You will weigh evidence using <em>null and alternative hypotheses, test statistics, and $p$-values</em>.
:::
